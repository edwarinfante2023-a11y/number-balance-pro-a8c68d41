  ...
  return (
    <div className="space-y-6 pt-2">
      <div className="mb-8 flex flex-col md:flex-row md:items-start justify-between gap-4">
         <div>
           <h1 className="text-[32px] font-bold tracking-tight text-foreground">Ingestión de Datos</h1>
           <p className="text-[15px] text-muted-foreground mt-1 max-w-2xl">Pipeline de carga masiva. Detecta dimensiones, consolida bloques horarios y auto-resuelve colisiones históricas.</p>
         </div>
         {parsed && step !== "done" && (
            <button 
              onClick={reset}
      ...
      {step === "done" && (
        <div className="bg-white rounded-[32px] p-16 text-center border border-border shadow-sm animate-fade-in relative overflow-hidden flex flex-col items-center justify-center min-h-[400px]">
          ...
          <h3 className="relative z-10 text-[24px] font-bold tracking-tight text-foreground">Consolidación Exitosa</h3>
          <p className="relative z-10 mt-3 text-[15px] text-muted-foreground max-w-md mx-auto leading-relaxed">
            El archivo ha sido indexado correctamente. Las asimetrías y paridades han sido insertadas en el Registry de forma persistente.
          </p>
          ...
          <div key={s.id} className="flex items-center shrink-0">
            <div
              className={cn(
                ...
                isPast ? "bg-muted/50 border border-border text-foreground" : "bg-transparent text-muted-foreground/50 border border-transparent"
              )}
            >
              <span className={cn(
            ...
            {i < steps.length - 1 && (
              <div className="w-8 h-[2px] bg-border mx-2 rounded-full" />
            )}
          </div>
        );
      })}
      ...
      >
        <div className="relative z-10 flex flex-col items-center justify-center py-24 text-center h-full">
          ...
          <div className={cn(
            ...
            busy ? "bg-white border-border shadow-inner" : "bg-white border-border group-hover:border-emerald-200 group-hover:bg-emerald-100/50 group-hover:scale-105"
          )}>
            {busy ? (
          ...
          >
            {busy ? (
              <span className="flex items-center gap-2"><Loader2 className="size-4 animate-spin"/> Mapeando...</span>
            ) : (
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      <div className="lg:col-span-8 bg-white rounded-[32px] border border-border shadow-sm overflow-hidden flex flex-col">
        <div className="px-8 py-6 border-b border-border bg-muted/20 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h3 className="text-[16px] font-bold text-foreground tracking-tight">Indexación de Schema</h3>
          <div className="flex flex-wrap gap-2">
            <span className="px-2.5 py-1 rounded-[6px] bg-white border border-border text-[11px] font-bold text-muted-foreground shadow-sm">{parsed.fileName}</span>
            <span className="px-2.5 py-1 rounded-[6px] bg-white border border-border text-[11px] font-bold text-muted-foreground shadow-sm">{parsed.totalRows.toLocaleString("es")} Rows</span>
            <span className="px-2.5 py-1 rounded-[6px] bg-white border border-border text-[11px] font-bold text-muted-foreground shadow-sm">{parsed.headers.length} Cols</span>
          </div>
        </div>

        <div className="p-8 space-y-3 flex-1 bg-muted/5">
          {FIELD_DEFS.map((f) => (
            <div key={f.key} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center p-4 rounded-[16px] bg-white border border-border shadow-sm hover:border-primary/30 transition-all group">
              <div className="flex flex-col">
                <div className="text-[14px] font-bold text-foreground flex items-center gap-2">
                  {f.label}
                  {f.required && <span className="text-[14px] leading-none text-red-500 mt-1">*</span>}
                  {f.manual && (
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="bg-white rounded-[32px] p-8 border border-border shadow-sm">
          ...
          <p className="text-[12px] font-medium text-muted-foreground mt-4 mb-6 leading-relaxed">
            Aplica un identificador de Red (Lotería) global si el dataset no contiene la columna de forma explícita.
          </p>
          <select
        ...
        {missingRequired.length > 0 ? (
            <div className="bg-orange-50 rounded-[24px] border border-orange-200 p-6 flex flex-col">
               ...
               <div>
                  ...
                  <div className="text-[12px] font-medium text-orange-800 leading-relaxed bg-white/50 p-3 rounded-xl border border-orange-200/50">
                    Requeridos que faltan: <strong className="block mt-1 font-mono">{missingRequired.map((f) => f.label).join(" / ")}</strong>
                  </div>
               </div>
            </div>
        ) : (
            <div className="bg-emerald-50 rounded-[24px] border border-emerald-200 p-6 flex items-center gap-4">
               ...
               <div className="text-[14px] font-bold text-emerald-900">Schema Autorizado.<br/><span className="text-[12px] font-medium text-emerald-700">Listo para Audit</span></div>
            </div>
        )}

        <div className="bg-white rounded-[24px] border border-border p-6 shadow-sm">
          <div className="text-[11px] font-bold uppercase tracking-[0.1em] text-muted-foreground mb-4">Buffer Columns</div>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <SummaryCard label="Muestra Válida" value={built.valid.length} tone="success" icon={<CheckCircle2 className="size-5" />} />
        <SummaryCard label="Corrupciones" value={built.errors.length} tone={built.errors.length > 0 ? "warning" : "muted"} icon={<AlertCircle className="size-5" />} />
        <SummaryCard label="Blocks (Total)" value={parsed.totalRows} tone="muted" icon={<Server className="size-5" /> } />
      </div>
          ...
          <table className="w-full text-left">
            <thead className="bg-muted/10 text-[11px] font-bold uppercase tracking-[0.1em] text-muted-foreground border-b border-border">
              <tr>
                ...
                <th className="px-8 py-4 whitespace-nowrap hidden sm:table-cell">Variables Conservadas</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {sample.map((r, i) => (
                <tr key={i} className="hover:bg-muted/30 transition-colors">
                  <td className="px-8 py-4 text-[13px] font-mono font-medium text-muted-foreground">{r.fecha}</td>
                  <td className="px-8 py-4 text-[13px] font-mono font-medium text-muted-foreground">{r.hora}</td>
                  <td className="px-8 py-4 text-[14px] font-bold text-foreground">{r.loteria}</td>
                  <td className="px-8 py-4">
                    <span className="font-mono text-[16px] font-extrabold text-foreground bg-muted px-3 py-1 rounded-[8px] border border-border">{String(r.numero).padStart(2,"0")}</span>
                  </td>
                  ...
                  <td className="px-8 py-4 hidden sm:table-cell">
                    {(() => {
                      const ma = (r.extra as { manual_analysis?: Record<string, unknown> }).manual_analysis;
                      if (!ma || Object.keys(ma).length === 0) {
                        return <span className="text-muted-foreground/30 font-mono text-[11px] font-bold">—</span>;
                      }
                      return (
              ...
              {sample.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-16 text-center text-muted-foreground bg-muted/10">
                     ...
                     <div className="text-[14px] font-bold text-orange-800">NULL SET. MAPEO INCORRECTO.</div>
                  </td>
                </tr>
              )}
            </tbody>
              ...
              <tbody className="divide-y divide-border">
                {built.errors.slice(0, 50).map((e, i) => (
                  <tr key={i} className="hover:bg-muted/30">
                    ...
                    <td className="py-3 px-8 text-[13px] font-medium text-foreground">{e.message}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function SummaryCard({
  ...
  return (
    <div className={cn(
       ...
       isS ? "bg-emerald-50 border-emerald-100" : isW ? "bg-orange-50 border-orange-100" : "bg-white border-border"
    )}>
      <div className="flex justify-between items-start mb-6">
         <div className={cn("text-[12px] font-bold uppercase tracking-[0.1em]", isS ? "text-emerald-800" : isW ? "text-orange-800" : "text-muted-foreground")}>{label}</div>
         {icon && <div className={cn("size-10 rounded-[12px] bg-white shadow-sm flex items-center justify-center", isS ? "text-emerald-500" : isW ? "text-orange-500" : "text-muted-foreground")}>{icon}</div>}
      </div>
      <div className={cn("text-[40px] font-extrabold tabular-nums leading-none", isS ? "text-emerald-700" : isW ? "text-orange-700" : "text-foreground")}>
...
function RecentImports({
  ...
  return (
    <div className="bg-white rounded-[32px] border border-border shadow-sm overflow-hidden mt-8">
      ...
      {imports.length === 0 ? (
        <div className="p-16 text-center text-[13px] font-bold text-muted-foreground/50 border-2 border-dashed border-border rounded-[24px] m-8">NO HAY LOGS DOCUMENTADOS</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-muted/10 text-[11px] font-bold uppercase tracking-[0.1em] text-muted-foreground border-b border-border">
              ...
              {imports.map((r) => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-8 py-4 text-[14px] font-mono font-semibold text-muted-foreground tabular-nums">{r.registros_duplicados}</td>
                    <td className="px-8 py-4 text-[14px] font-mono font-bold text-orange-600 tabular-nums">{r.errores}</td>
                    <td className="px-8 py-4">
                      <span
                        className={cn(
    ...
    <div className="bg-white border border-border shadow-sm rounded-[32px] p-8 relative overflow-hidden mb-8">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-[14px] text-muted-foreground mt-2 max-w-2xl leading-relaxed">
            Múltiples sub-grupos detectados. El importador unificará y paralelizará este batch masivo inteligentemente.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 max-h-64 overflow-y-auto custom-scrollbar pr-2">
        {parsed.blocks.map((b, i) => (
          <div key={i} className="bg-muted/30 rounded-[16px] border border-border p-4 text-xs flex flex-col justify-between hover:bg-muted/60 transition-colors">
            <div className="font-bold text-foreground truncate text-[13px] font-mono mb-4 bg-white px-2 py-1 rounded inline-flex shadow-sm border border-border/50">{b.label}</div>
            <div className="flex items-center justify-between mt-auto">
              <span className="text-[13px] font-bold text-muted-foreground tabular-nums">{b.rows.length} rows</span>
              {b.contextoFecha && (
