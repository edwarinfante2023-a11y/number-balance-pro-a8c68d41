...
import { Plus, Trash2, Loader2, Save, ChevronDown, ChevronRight, Clock, ShieldAlert, Cpu, Network, Sparkles, FolderTree } from "lucide-react";
  ...
  return (
    <div className="space-y-6 pt-2">
      <div className="mb-8">
         ...
         <p className="text-[15px] text-muted-foreground mt-1">Variables Core y Topología de Red. Alterar estos umbrales recalibra la entropía predictiva.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Core Rules Engine */}
        <div className="lg:col-span-7 bg-white rounded-[32px] border border-border shadow-sm p-8 relative overflow-hidden flex flex-col">
          <div className="flex items-center justify-between mb-8 border-b border-border pb-6 relative z-10">
            <div className="flex items-center gap-3">
               ...
               <div>
                 ...
                 <p className="text-[12px] font-semibold text-muted-foreground uppercase tracking-widest mt-0.5">Core Rules</p>
               </div>
            </div>
            {dirty && (
              <button
          ...
          {isLoading ? (
            <div className="text-[13px] font-bold text-muted-foreground flex items-center justify-center py-10">
              <Loader2 className="size-5 animate-spin inline mr-3 text-primary" /> Cargando kernel...
            </div>
          ) : (
            <div className="relative z-10 space-y-6">
              ...
              <div className="bg-muted/30 rounded-[20px] p-6 border border-border mt-6">
                <Field
                  label={`OVERRIDE: LIMITER A/B (≥ ${current.altoThreshold})`}
                >
                  <input
                    type="range"
                  ...
                  <div className="flex justify-between text-[11px] font-bold text-muted-foreground tabular-nums mt-6 border-t border-border pt-4">
                    <span className="flex flex-col gap-1">
                      <span className="uppercase text-[10px] text-bajo tracking-widest drop-shadow-sm">Bajo Vector</span>
                      <span className="text-[14px]">[{current.rangeMin} — {current.altoThreshold - 1}]</span>
                    </span>
                    <span className="flex flex-col gap-1 text-right">
                      <span className="uppercase text-[10px] text-alto tracking-widest drop-shadow-sm">Alto Vector</span>
                      <span className="text-[14px]">[{current.altoThreshold} — {current.rangeMax}]</span>
                    </span>
                  </div>
                </Field>
              </div>
              ...
              <div className="mt-4 rounded-[16px] bg-warning/10 border border-warning/20 p-5 text-[13px] text-orange-800 font-medium flex gap-4 items-start shadow-sm">
                ...
                <div className="leading-relaxed">
                  PAR/IMPAR se aisla computando el último dígito del buffer. 
                  Mutar el <strong className="font-bold text-orange-900 border-b border-orange-900/30">LIMITER A/B</strong> no recompila eventos pasados automáticamente. Requiere purgado manual.
                </div>
              </div>
            </div>
          )}
        ...
        <div className="lg:col-span-5 bg-white rounded-[32px] border border-border shadow-sm p-8 relative flex flex-col h-full max-h-[800px]">
            ...
            <div>
              <div className="flex items-center gap-3">
                 ...
                 <div>
                   ...
                   <p className="text-[12px] font-semibold text-muted-foreground uppercase tracking-widest mt-0.5">Nodos y Bloques</p>
                 </div>
              </div>
            </div>
          </div>
          ...
          <ul className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
            {lotteries.length === 0 && (
              <li className="text-[12px] font-bold text-muted-foreground/50 text-center py-16 uppercase border-2 border-dashed border-border rounded-[20px]">Topología vacía</li>
            )}
            {lotteries.map((l) => (
              <LotteryItem
                key={l.id}
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="max-w-2xl">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                   <h3 className="text-[24px] font-bold tracking-tight text-white leading-none">Motor Neuronal Predictivo</h3>
                   <div className="flex items-center gap-2 mt-2">
                     <span className="size-2 rounded-full bg-white animate-pulse" />
                     <span className="text-[12px] font-bold uppercase tracking-[0.2em] text-white/80">Coming Soon</span>
                   </div>
                </div>
              </div>
              <p className="text-[14px] text-white/80 leading-relaxed font-medium mt-3">
                El core AI analizará todo tu historial (matriz X/Y) para someter tus reglas a estrés algorítmico, revelando vectores anómalos ocultos y prediciendo asimetrías con rangos de confianza explícitos. No es un chatbot, es matemáticas puras.
              </p>
            </div>
            ...
            <div className="shrink-0 grid grid-cols-1 sm:grid-cols-3 gap-3 w-full md:w-auto">
              {[
                ...
                <div key={c.t} className="rounded-[16px] bg-white/10 backdrop-blur-md border border-white/20 p-4 shadow-inner">
...
function LotteryItem({
  ...
  return (
    <li className="rounded-[20px] border border-border bg-white shadow-sm overflow-hidden group">
      <div className="flex items-center justify-between px-5 py-4 bg-white hover:bg-muted/30 transition-colors border-b border-transparent data-[state=open]:border-border cursor-pointer select-none" data-state={open ? "open" : "closed"} onClick={() => setOpen(!open)}>
              ...
              {lottery.draws.map((d) => (
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="font-extrabold text-[14px] tabular-nums text-foreground">{d.hora}</span>
                  </div>
                  <button
                    onClick={async () => {
